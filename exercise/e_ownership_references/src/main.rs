...
fn inspect(input_str: &String) {
    let result = if input_str.ends_with('s') {
        "Ends with s!"
    } else {
        "Doesn't end with s!"
    }
...
fn change(input_str: &mut String) {
    if !input_str.ends_with('s') {
        input_str.push_str("s")
    }
}

fn eat(input_str: String) -> bool {
    input_str.starts_with("b") && input_str.contains("a")
}

fn bedazzle(input_str: &mut String) {
    *input_str = String::from("sparkly")
}
